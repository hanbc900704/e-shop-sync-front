<template>
    <div class="flex w-full flex-col px-[24px] py-[12px]">
        <div class="mb-[18px] flex flex-wrap items-center justify-between">
            <span class="ht-18 text-[#1D2939]">Track Bill</span>
            ...
            <div class="mb-[18px] flex flex-wrap items-center justify-between">
                <div class="flex w-[500px] flex-col">
                    <div class="flex gap-3">
                        <span class="ht-18 mb-[16px] text-[#1D2939]">{{ bill?.bill_no || "" }}</span>
                        <span class="ht-18 mb-[16px] text-[#1D2939]">{{ bill?.short_title || "" }}</span>
                    </div>
                    <div class="mb-[8px] flex w-full flex-wrap">
                        <span class="ht-14 mr-[16px] w-[100px] text-[#667085]">Affected Titles: </span>
                        <span class="ht-14 text-[#667085]">{{ bill.titles }}</span>
                    </div>
                    <div class="flex w-full flex-wrap">
                        <span class="ht-14 mr-[16px] w-[100px] text-[#667085]">Last Action: </span>
                        <span class="ht-14 text-[#667085]">{{ bill.last_action }} {{ bill.actionDate }}</span>
                    </div>
                    <div class="my-2 flex w-full flex-wrap">
                        <span class="ht-14 mr-[16px] w-[100px] text-[#667085]">Progress: </span>
                        <span class="ht-14 text-[#667085]">{{ bill.progress }}</span>
                    </div>
                </div>
                <div class="inline-grid grid-cols-2 items-center gap-2">
                    <UButton color="white" variant="solid" icon="i-mdi-link" class="!border-none bg-transparent ring-0" />
        ...
        <div class="flex w-full flex-wrap items-end justify-between border-b-2 py-[12px] pt-[24px]">
            ...
            <UButton color="white" class="border-1 border-black bg-transparent px-[13px] py-[3px]"
                @click="toggleAddTracking">
                Create New List
            </UButton>
        </div>

        <div v-if="bill.lists" class="w-full pt-[24px]">
            <UTable v-model="selectedRows" class="rounded-lg border border-[#EAECF0]" :rows="filteredRows"
...
export default {
    name: "BillRight",
    inject: ['istrackingPaneOpen', 'toggleAddTracking'],
    ...
    computed: {
        affectedTitles() {
            console.log("******** right page", this.filteredRows)
            if (this.bill.titles) {
                return Array.from(this.bill.titles).join(", ");
            }
            return "";
        },
        // listCnt() {
